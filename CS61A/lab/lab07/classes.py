...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if implemented:
            ...
            player.hand.extend([player.deck.cards.pop(0), player.deck.cards.pop(1)])
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) != 0:
            ...
            for i in range(1, len(player.hand)):
                curr_power = player.hand[i].power(opponent_card)
                if curr_power > max_power:
                    max_power = curr_power
                    card_index = i
            best_card = player.hand[card_index]
