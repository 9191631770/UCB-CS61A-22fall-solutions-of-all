...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        bound = 0
        while curr_place.is_hive is False:
            if curr_place.bees != [] and self.lower_bound <= bound <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        is_dead = False
        bees_place = self.place.bees
        Ant.reduce_health(self, amount)
        if self.health <= 0:
            is_dead = True
        len_bees = len(bees_place)
        i = 0
        for _ in range(len_bees):
            temp_bee = bees_place[i]
            if is_dead:
                Ant.reduce_health(bees_place[i], amount + self.damage)
            else:
                Ant.reduce_health(bees_place[i], amount)
            if temp_bee.health > 0:
                i += 1
...
# BEGIN Problem 7
class HungryAnt(Ant): # The HungryAnt Class
    name = 'Hungry'
    food_cost = 4
    implemented = True
    chewing_turns = 3
...
# BEGIN Problem 9
class TankAnt(ContainerAnt): # The TankAnt class
    name = 'Tank'
    food_cost = 6
    damage = 1
    implemented =True
    ...
    def action(self, gamestate):
        ...
        for _ in range(len_bees):
            temp_bee = bees_place[i]
            Ant.reduce_health(bees_place[i], self.damage)
            if temp_bee.health > 0:
                i += 1
    ...
    def add_insect(self, insect):
        ...
        if insect.is_waterproof is False:
            while insect.health != 0:
                insect.reduce_health(1)
        # END Problem 10

# BEGIN Problem 11
class ScubaThrower(ThrowerAnt): # The ScubaThrower class
    name = 'Scuba'
    food_cost = 6
    is_waterproof = True
    implemented = True
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.has_queen is True:
            return None
    ...
    def action(self, gamestate):
        ...
        while curr_place is not None:
            if curr_place.ant is not None:
                # if curr_place.ant.name != 'Fire':
                curr_place.ant.double()
                if curr_place.ant.is_container is True:
                    if curr_place.ant.ant_contained is not None:
...
class LaserAnt(ThrowerAnt, ContainerAnt):
    ...
    def insects_in_front(self):
        ...
        while curr_place is not None:
            if curr_place.ant is not None:
                dict[curr_place.ant] = distance
                if curr_place.ant.is_container:
                    if curr_place.ant.ant_contained is not None:
                        dict[curr_place.ant.ant_contained] = distance
